...
public class SinglePositionModel {
    // collection_
    public static final String COLLECTION_PRE = "position_";
...
//    private String  imState; //                      "imState": "threeDays",
    ...
    // 薪水上下限   "salary": "20k-25k"
    private Integer salaryBegin; // 薪水开始
    private Integer salaryEnd;  // 薪水结束

    /**
     * 根据薪水值生成上下限
     */
    public void parserSalary(){
        if(salary != null){
            // salary: 8k-12k; 8K-12K; 8k以上
            System.out.println(salary);
            if(salary.indexOf("-") < 0){ // 8k以上
                System.out.println("不处理" + salary);
                return;
            }
            // 最小值
            this.salaryBegin = Integer.parseInt(salary.substring(0, salary.indexOf("-") - 1));
            // 最大值
            this.salaryEnd = Integer.parseInt(salary.substring(salary.indexOf("-") + 1, salary.length() - 1));
        }
    }
