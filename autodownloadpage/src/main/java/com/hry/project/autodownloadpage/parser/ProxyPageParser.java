...
import com.hry.project.autodownloadpage.http.HttpProxyCheck;
...
public class ProxyPageParser {
	...
	@Autowired
	private HttpProxyCheck httpProxyCheck;

	public void parser()throws IOException, InterruptedException{
		int num = 100;
		for(int i = 1; i <= num; i++){
		//	String url = "http://www.kuaidaili.com/free/outtr/"+i+"/";
	...
	//	String url = "http://www.kuaidaili.com/free/outtr/1/";
		...
		for (Element ele : trList) {
			...
			try{
				httpProxyCheck.checkProxyWork(ip, Integer.parseInt(port));
				logger.info("good proxy {}, {}",ip, port);
			}catch(Exception e){
